...
class ReceivedCoupon extends BaseModel
{
    ...
    *******************************************/
    ...
    private function get_datatable_query()
    {
        ...
        $this->column_order = ['id',null, 'customer_id', 'salesmen_id', null, null, 'created_at','coupon_price'];
        
        $query = self::with('salesmen','customer','coupon')->where('asm_id',Auth::guard('asm')->user()->id);

        //search query
        if (!empty($this->_from_date)) {
            $query->where('created_at', '>=',$this->_from_date.' 00:00:01');
        }
        if (!empty($this->_to_date)) {
            $query->where('created_at', '<=',$this->_to_date.' 23:59:59');
        }
