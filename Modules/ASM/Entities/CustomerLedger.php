...
class CustomerLedger extends BaseModel
{
    ...
    *******************************************/
    ...
    private function get_datatable_query()
    {
        ...
        $query = self::select('transactions.*','coa.id as coa_id','coa.code','coa.name','coa.parent_name','c.id as customer_id','c.shop_name','c.name','c.mobile','c.district_id')
        ->join('chart_of_accounts as coa','transactions.chart_of_account_id','=','coa.id')
        ->join('customers as c','coa.customer_id','c.id')
        ->where(['c.district_id'=> Auth::guard('asm')->user()->district_id,'transactions.approve'=>1]);
    ...
    {
        return self::select('transactions.*','coa.id as coa_id','coa.code','coa.name','coa.parent_name','c.id as customer_id','c.shop_name','c.name','c.mobile','c.district_id')
        ->join('chart_of_accounts as coa','transactions.chart_of_account_id','=','coa.id')
        ->join('customers as c','coa.customer_id','c.id')
        ->where(['c.district_id'=> Auth::guard('asm')->user()->district_id,'transactions.approve'=>1])->get()->count();
    }
    /******************************************
