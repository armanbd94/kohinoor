...
use Modules\ASM\Entities\DailyClosing;
...
class ClosingReportController extends BaseController
{
    ...
    public function index()
    {
        ...
        $this->setPageData('Closing Account','Closing Account','fas fa-file-signature',[['name' => 'Report'],['name' => 'Closing Account']]);
        $last_closing_amount_data = DailyClosing::select('amount')->where(['warehouse_id' => Auth::guard('asm')->user()->warehouse_id])->latest('date')->first();
        $cash_data=  DB::table('transactions as t')
                    ...
                    ->where(['t.warehouse_id' => Auth::guard('asm')->user()->warehouse_id,'t.voucher_date' => date('Y-m-d'),'coa.code' => $this->coa_head_code('cash_in_hand')])
                    ->first();

        $last_closing_amount  = $last_closing_amount_data ? $last_closing_amount_data->amount : 0;
        $cash_in = $cash_data ? $cash_data->cash_in_amount : 0;
        $cash_out = $cash_data ? $cash_data->cash_out_amount : 0;
        ...
        return view('asm::report.closing-report.form',$data);
    ...
    public function store(Request $request)
    {
        if($request->ajax())
        {
            $closing_data = DB::table('daily_closings')->where(['date' => date('Y-m-d'),'warehouse_id' => Auth::guard('asm')->user()->warehouse_id])->get()->count();
            if($closing_data > 0)
            {
    ...
    public function report()
    {
        ...
        return view('asm::report.closing-report.report');
    }
