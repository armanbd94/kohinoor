        ...
        if($request->ajax())
        {
            if(!empty($request->search))
            {
                ...
                $data =  WarehouseFinishGoods::with('product','finish_goods')->where([
                    [ 'warehouse_id',Auth::guard('asm')->user()->warehouse_id],['qty','>',0]
                ...
                // ->WhereHas('finish_goods',function($q) use ($search_text){
                //     $q->where('code','like','%'.$search_text.'%');
                // })->orWhereHas('product',function($q) use ($search_text){
        ...
        if($request->ajax())
        {
            ...
            if($finish_goods_data)
            {
                ...
                $warehouse_product = WarehouseFinishGoods::where([
                    'warehouse_id'=> Auth::guard('asm')->user()->warehouse_id,'product_id'=>$finish_goods_data->product_id,'finish_goods_id'=>$finish_goods_data->id])->first();
