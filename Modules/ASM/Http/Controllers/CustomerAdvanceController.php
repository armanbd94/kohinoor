...
use Modules\ASM\Entities\CustomerAdvance;
...
class CustomerAdvanceController extends BaseController
{
    ...
    {
        $this->setPageData('Customer Advance','Customer Advance','fas fa-hand-holding-usd',[['name'=>'Customer','link'=>route('asm.customer')],['name'=>'Customer Advance']]);
        $customers = Customer::with('coa')->where(['status'=>1,'district_id'=> Auth::guard('asm')->user()->district_id])->orderBy('name','asc')->get();
        return view('asm::customer.advance.index',compact('customers'));
    }
    ...
    public function get_datatable_data(Request $request)
    {
        if($request->ajax()){
            ...
            foreach ($list as $value) {
                ...
                $row[] = $no;
                $row[] = $value->name.' - '.$value->mobile.' ('.$value->shop_name.')';
    ...
    {
        if($request->ajax()){
            $data   = $this->model->select('transactions.*','coa.id as coa_id','coa.code','c.id as customer_id')->join('chart_of_accounts as coa','transactions.chart_of_account_id','=','coa.id')
