        ...
        <div class="card card-custom gutter-b">
            <div class="card-header flex-wrap py-5">
                ...
                <div class="card-toolbar">
                    <!--begin::Button-->
                    <a href="{{ route('asm.sale') }}" class="btn btn-warning btn-sm font-weight-bolder"> 
                ...
                <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                    <form id="sale_update_form" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <input type="hidden" name="salesmen_id_hidden" value="{{ $sale->salesmen_id }}">
                            <input type="hidden" name="route_id_hidden" value="{{ $sale->customer->route_id }}">
                            ...
                            <x-form.selectbox labelName="Route" name="route_id" col="col-md-4" required="required" class="selectpicker">
                                @if (!$routes->isEmpty())
                                    ...
                                    '<option value="{{ $value->route_id }}">{{ DAYS[$value->day].' - '.$value->route->name }}</option>
                                    @endforeach 
                                @endif
                            </x-form.selectbox>
                                    ...
                                    <tbody>
                                        ...
                                        @if (!$sale->sale_products->isEmpty())
                                            @foreach ($sale->sale_products as $key => $sale_product)
                                                <tr>
                                                    @php
                                                        $tax = DB::table('taxes')->where('rate',$sale_product->product->tax_rate)->first();
                                                        ...
                                                        ->where(['warehouse_id'=>$sale->warehouse_id,'product_id'=>$sale_product->product_id,'finish_goods_id'=>$sale_product->id])->value('qty');
                                                        ...
                                                        if($units){
                                                            ...
                                                            if($sale_product->product->tax_method == 1){
                                                    ...
                                                    @endphp
                                                    <td>{{ $sale_product->product->product_name.' '.$sale_product->size->size.$sale_product->size->unit->unit_name }}</td>
                                                    ...
                                                    @endif
                                                    <input type="hidden" class="tax-method" value="{{ $sale_product->product->tax_method }}">
                            ...
                            <div class="form-group col-md-2">
                                ...
                                <input type="text" class="form-control" name="labor_cost" id="labor_cost"  value="{{ $sale->total_labor_cost }}">
                            </div>
...
$(document).ready(function () {
    ...
    for (rowindex = 0; rowindex <= rownumber; rowindex++) {
        ...
        var total_discount = parseFloat($('#product_table tbody tr:nth-child('+ (rowindex + 1) +')').find('td:nth-child(6)').text())
    ...
    //assigning value
    $('select[name="salesmen_id"]').val($('input[name="salesmen_id_hidden"]').val());
    $('select[name="route_id"]').val($('input[name="route_id_hidden"]').val());
    ...
    $.get('{{ url("asm/customer/group-data") }}/'+cid,function(data){
    ...
    $('#salesmen_id').on('change',function(){
        var id = $(this).val();
        $.ajax({
            url:"{{route('asm.sales.representative.daily.route.list')}}",
            type: 'post',
    ...
    $('#customer_id').on('change',function(){
        var id = $(this).val();
        $.get('{{ url("asm/customer/group-data") }}/'+id,function(data){
            customer_group_rate = (data/100);
        });
        $.get('{{ url("asm/customer/previous-balance") }}/'+id,function(data){
    ...
    $('#product_code_name').autocomplete({
        ...
        source: function( request, response ) {
          // Fetch data
          $.ajax({
            url:"{{url('asm/product-autocomplete-search')}}",
            type: 'post',
            dataType: "json",
            data: {
               _token: _token,
            ...
            success: function (data) {
                ...
                if (data.status == false) {
                    ...
                    if (data.status == 'success') {
                        window.location.replace("{{ route('asm.sale') }}");
