            ...
            <div class="col-xl-3">
                ...
                background-image: url({{ asset('images/abstract-1.svg') }})">
                    ...
                    <div class="card-body">
                        ...
                        <span class="card-title font-weight-bolder text-white-75 font-size-h2 mb-0 mt-6 d-block  text-white"  id="total-salesmen">0</span>
                        <span class="font-weight-bold font-size-sm  text-white">Total Salesmen</span>
                    </div>
                    <!--end::Body-->
                </div>
                <!--end::Stats Widget 29-->
            ...
            <div class="col-xl-3">
                ...
                background-image: url({{ asset('images/abstract-1.svg') }})">
                    ...
                    <div class="card-body">
                        ...
                        <span class="card-title font-weight-bolder text-white-75 font-size-h2 mb-0 mt-6 d-block  text-white"  id="total-coupon">0Tk</span>
                        <span class="font-weight-bold font-size-sm  text-white">Total Coupon Received</span>
                    </div>
                    <!--end::Body-->
                </div>
                <!--end::Stats Widget 29-->
...
function show_data(value='') {
    $.ajax({
        url: "{{route('asm.dashboard.data')}}",
        type: "POST",
        data: {value:value,_token:"{{ csrf_token() }}"},
        dataType: "JSON",
        success: function (data) {
            data.total_customer ? $("#total-customer").text(data.total_customer) : $("#total-customer").text('0');
            data.total_salesmen ? $("#total-salesmen").text(data.total_salesmen) : $("#total-salesmen").text('0');
            data.total_sale ? $("#total-sale").text(number_format(data.total_sale+'TK')) : $("#total-sale").text('0.00Tk');
            data.total_coupon ? $("#total-coupon").text(number_format(data.total_coupon+'Tk')) : $("#total-coupon").text('0.00Tk');
        },
