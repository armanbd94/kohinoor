        ...
        <div class="card card-custom gutter-b">
            <div class="card-header flex-wrap py-5">
                ...
                <div class="card-toolbar">
                    <!--begin::Button-->
                    @if (permission('sales-representative-add'))
            ...
            <div class="card-header flex-wrap py-5">
                <form method="POST" id="form-filter" class="col-md-12 px-0">
                    <div class="row">
                        ...
                        <x-form.selectbox labelName="ASM" name="asm_id" required="required" col="col-md-4" class="selectpicker" onchange="getUpazilaList(1)">
                            @if (!$asms->isEmpty())
                            @foreach ($asms as $asm)
                                <option value="{{ $asm->id }}" data-district="{{ $asm->district_id }}">{{ $asm->name.' - '.$asm->warehouse->name }}</option>
                            @endforeach 
                            @endif
                        </x-form.selectbox>
    ...
    table = $('#dataTable').DataTable({
        ...
        "ajax": {
            ...
            "data": function (data) {
                ...
                data.asm_id    = $("#form-filter #asm_id option:selected").val();
        ...
        "columnDefs": [{
            ...
            {
                @if (permission('sales-representative-bulk-delete'))
                "targets": [1,2,4,5,6,7,8,10],
                @else
                "targets": [0,1,3,4,5,6,7,9],
                @endif
        ...
        "buttons": [
            @if (permission('sales-representative-report'))
            {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.status == 'error'){
                        ...
                        $('#store_or_update_form #asm_id').val(data.asm_id);
...
{
    var district_id = (set_id == 1) ? $('#form-filter #asm_id option:selected').data('district') : $('#store_or_update_form #asm_id option:selected').data('district');
