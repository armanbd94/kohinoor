...
class CreditCustomer extends BaseModel
{
    ...
    *******************************************/
    ...
    private function get_datatable_query()
    {
        ...
        $this->column_order = ['c.id','c.name', 'c.shop_name','c.address','c.mobile','c.district_id','c.upazila_id','c.route_id','c.customer_group_id',null];
        
        
        $query = DB::table('customers as c')
        ->selectRaw('c.*,d.name as district_name,u.name as upazila_name,r.name as route_name,cg.group_name, ((select ifnull(sum(debit),0) from transactions where chart_of_account_id= b.id)-(select ifnull(sum(credit),0) from transactions where chart_of_account_id= b.id)) as balance')
