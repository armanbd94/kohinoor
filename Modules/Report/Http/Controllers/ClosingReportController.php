...
class ClosingReportController extends BaseController
{
    ...
    public function index()
    {
        ...
        if(permission('closing-access')){
            ...
            return view('report::closing-report.form',compact('warehouses'));
        }else{
    ...
    {
        if ($request->ajax()) {
            $last_closing_amount_data = DailyClosing::select('amount')->where(['warehouse_id' => $request->warehouse_id])->latest('date')->first();
            $cash_data=  DB::table('transactions as t')
                        ...
                        ->where(['t.warehouse_id' => $request->warehouse_id,'t.voucher_date' => date('Y-m-d'),'coa.code' => $this->coa_head_code('cash_in_hand')])
                        ->first();
    ...
    public function report()
    {
        if(permission('closing-report-access')){
            ...
            $warehouses = Warehouse::where('status',1)->get();
            return view('report::closing-report.report',compact('warehouses'));
        }else{
            return $this->access_blocked();
        }
    }
    ...
    public function get_datatable_data(Request $request)
    {
        if($request->ajax()){
            ...
            foreach ($list as $value) {
                ...
                $row = [];
                $row[] = $no;
                $row[] = DB::table('warehouses')->where('id',$value->warehouse_id)->value('name');
