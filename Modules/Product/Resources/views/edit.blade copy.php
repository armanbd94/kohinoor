                    ...
                    <div class="row">
                        <div class="col-md-10">
                            <div class="row">
                                ...
                                <div class="form-group col-md-4 required unit">
                                    <label for="code">Purchase Unit</label>
                                    <select name="purchase_unit_id" id="purchase_unit_id"  class="form-control selectpicker" data-live-search="true"  data-live-search-placeholder="Search">
                                    @php
                                    ...
                                    @if (!$sale_units->isEmpty())
                                        ...
                                        <option value="{{ $unit->id }}" {{ ($product->purchase_unit_id == $unit->id) ? 'selected' : '' }}>{{ $unit->unit_name.' ('.$unit->unit_code.')' }}</option>
                                        @endforeach
                                    @endif
                                    </select>
                                </div>
                                ...
                                <x-form.textbox labelName="Product Cost" name="cost" required="required" value="{{ $product->cost }}" col="col-md-4 cost" placeholder="Enter product cost"/>
                                <x-form.textbox labelName="Product Price" name="price" value="{{ $product->type == 1 ? $product->price : '' }}" required="required" col="col-md-4 price {{ $product->type == 2 ? 'd-none' : '' }}" placeholder="Enter product price"/>
                                <x-form.textbox labelName="Alert Quantity" name="alert_quantity" value="{{ $product->alert_quantity }}"  col="col-md-4 alert-qty" placeholder="Enter product alerty qty"/>
                                ...
                                <div class="col-md-4 form-group required">
                                    <label for="subtract_stock">Subtrtact Stock</label>
                                    <select name="subtract_stock" id="subtract_stock"  class="form-control selectpicker">
                                        @foreach (SUBTRACT_STOCK as $key => $value)
                                            <option value="{{ $key }}" {{ $product->subtract_stock == $key ? 'selected' : '' }}>{{ $value }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-md-12 {{ $product->type == 1 ? 'd-none' : '' }} pt-5" id="variant-section">
                            ...
                            <div class="table-responsive mt-5" id="variant-list">
                                <table id="variant-table" class="table table-bordered table-hover variant-list">
                                    <thead class="bg-primary">
                                        <th class="text-center" width="5%"><i class="fas fa-th text-white"></i></th>
                                        <th width="40">Name</th>
                                        <th width="25%">Item Code</th>
                                        <th width="20$" class="text-right">Price</th>
                                        <th width="10%" class="text-center"><i class="fas fa-trash text-white"></i></th>
                                    </thead>
                                    <tbody>
                                        @if($product->type == 2 && !$product->variants->isEmpty())
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="form-group mb-0">
                                                    <input type="text" class="form-control text-right" name="variants[{{ $key+1 }}][price]" id="variants_{{ $key+1 }}_price" value="{{ $value->item_price }}" data-row="{{ $key+1 }}">
                                                    </div>
                                                </td>
                        ...
                        <div class="col-md-12 mt-3">
                            <input name="promotion" type="checkbox" id="promotion" value="{{ $product->promotion}}" {{ $product->promotion == 1  ? 'checked' : ''}}>&nbsp;
                            <label><h5> Add Promotional Price</h5></label>
                        </div>
                        <div class="col-md-12 promotion-section {{ $product->promotion == 2  ? 'd-none' : ''}}">
                            <div class="row">
                                <div class="form-group col-md-4 required">
                                    <label>Promotional Price</label>
                                    <input type="text" name="promotion_price" id="promotion_price" value="{{ $product->promotion_price ?? '' }}" class="form-control" >
                                </div>
                                <div class="form-group required col-md-4">
                                    <label>Promotion Starts</label>
                                    <div class="input-group">
                                        <input type="text" name="start_date" id="start_date"  value="{{ $product->start_date ?? '' }}" class="form-control date" readonly>
                                        <div class="input-group-prepend">
                                            <div class="input-group-text bg-primary"><i class="far fa-calendar-alt text-white"></i></div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4 form-group required">
                                    ...
                                    <div class="input-group">
                                        <input type="text" name="end_date" id="end_date"   value="{{ $product->end_date ?? '' }}" class="form-control date" readonly>
                                        <div class="input-group-prepend">
                                            <div class="input-group-text bg-primary"><i class="far fa-calendar-alt text-white"></i></div>
                                        </div>
                                    </div>
