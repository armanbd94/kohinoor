                ...
                <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                    <form action="" id="transfer_form" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <input type="hidden" name="from_warehouse_id_hidden" id="from_warehouse_id_hidden" value="{{ $transfer->from_warehouse_id }}">
                            <input type="hidden" name="to_warehouse_id_hidden" id="to_warehouse_id_hidden" value="{{ $transfer->to_warehouse_id }}">
                            <input type="hidden" name="transfer_status_hidden" id="transfer_status_hidden" value="{{ $transfer->transfer_status }}">
                            <input type="hidden" name="received_by_hidden" id="received_by_hidden" value="{{ $transfer->received_by }}">
                            ...
                            <div class="form-group col-md-3 required">
                                ...
                                <input type="text" class="form-control date" name="transfer_date" id="transfer_date" value="{{ $transfer->transfer_date }}" readonly />
                            </div>
                            ...
                            <div class="col-md-12">
                                <table class="table table-bordered" id="product_table">
                                    ...
                                    <tbody>
                                        ...
                                        @if (!$transfer->transfer_products->isEmpty())
                                            @foreach ($transfer->transfer_products as $key => $transfer_product)
                                                <tr>
                                                    @php
                                                        $tax = DB::table('taxes')->where('rate',$transfer_product->product->tax_rate)->first();

                                                        $units = DB::table('units')->where('base_unit',$transfer_product->unit_id)
                                                                                    ->orWhere('id',$transfer_product->unit_id)
                                                                                    ->get();
                                                        $warehouse_qty = DB::table('warehouse_finish_goods')
                                                        ->where(['warehouse_id'=>$transfer->warehouse_id,'product_id'=>$transfer_product->product_id,'finish_goods_id'=>$transfer_product->id])->value('qty');
                                                        $stock_qty = ($warehouse_qty ? $warehouse_qty : 0) + $transfer_product->pivot->qty;
                                                        ...
                                                        if($units){
                                                            foreach ($units as $unit) {
                                                                if($transfer_product->pivot->unit_id == $unit->id)
                                                                {
                                                            ...
                                                            if($transfer_product->product->tax_method == 1){
                                                                $product_cost = $transfer_product->pivot->net_unit_cost / $unit_operation_value[0];
                                                            }else{
                                                                $product_cost = ($transfer_product->pivot->total / $transfer_product->pivot->qty) / $unit_operation_value[0];
                                                            }
                                                    ...
                                                    @endphp
                                                    <td>{{ $transfer_product->product->product_name.' '.$transfer_product->size->size.$transfer_product->size->unit->unit_name }}</td>
                    
                                                    <td>{{ $transfer_product->code }}</td>
                                                    <td class="unit-name"></td>
                                                    <td><input type="text" class="form-control qty text-center" name="products[{{ $key+1 }}][qty]"
                                                        id="products_{{ $key+1 }}_qty" value="{{ $transfer_product->pivot->qty }}"></td>

                                                    <td class="net_unit_cost text-right">{{ number_format((float)$transfer_product->pivot->net_unit_cost, 2, '.','') }}</td>
                                                    <td class="tax text-right">{{ number_format((float)$transfer_product->pivot->tax, 2, '.','') }}</td>
                                                    <td class="sub-total text-right">{{ number_format((float)$transfer_product->pivot->total, 2, '.','') }}</td>
                                                    <td class="text-center"><button type="button" class="edit-product btn btn-sm btn-primary mr-2" data-toggle="modal"
                                                    ...
                                                    <input type="hidden" class="product-id" name="products[{{ $key+1 }}][id]"  value="{{ $transfer_product->id }}">
                                                    <input type="hidden" class="stock-qty" name="products[{{ $key+1 }}][stock_qty]"  value="{{ $stock_qty }}">
                                                    <input type="hidden" class="product-code" name="products[{{ $key+1 }}][code]" value="{{ $transfer_product->code }}">
                                                    <input type="hidden" class="product-cost" name="products[{{ $key+1 }}][cost]" value="{{ $product_cost }}">
                                                    <input type="hidden" class="transfer-unit" name="products[{{ $key+1 }}][unit]" value="{{ $unit_name }}">
                                                    ...
                                                    <input type="hidden" class="net_unit_cost" name="products[{{ $key+1 }}][net_unit_cost]" value="{{ $transfer_product->pivot->net_unit_cost }}">
                                                    <input type="hidden" class="tax-rate" name="products[{{ $key+1 }}][tax_rate]" value="{{ $transfer_product->pivot->tax_rate }}">
                                                    @if ($tax)
                                                    <input type="hidden" class="tax-name" value="{{ $tax->name }}">
                                                    @else
                                                    <input type="hidden" class="tax-name" value="No Tax">
                                                    @endif
                                                    <input type="hidden" class="tax-method" value="{{ $transfer_product->product->tax_method }}">
                                                    <input type="hidden" class="tax-value" name="products[{{ $key+1 }}][tax]" value="{{ $transfer_product->pivot->tax }}">
                                                    <input type="hidden" class="subtotal-value" name="products[{{ $key+1 }}][subtotal]" value="{{ $transfer_product->pivot->total }}">
                                    ...
                                    <tfoot class="bg-primary">
                                        ...
                                        <th></th>
                                        <th id="total-tax" class="text-right font-weight-bolder">{{ number_format($transfer->total_tax,2,'.',',') }}</th>
                                        <th id="total" class="text-right font-weight-bolder">{{ number_format($transfer->total_cost,2,'.',',') }}</th>
                                        <th></th>
                                    </tfoot>
                                </table>
                            </div>
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <input type="text" class="form-control" name="shipping_cost" id="shipping_cost" value="{{ $transfer->shipping_cost }}">
                            </div>
                            <div class="form-group col-md-4">
                                <label for="labor_cost">Labor Cost</label>
                                <input type="text" class="form-control" name="labor_cost" id="labor_cost"  value="{{ $transfer->total_labor_cost }}">
                            </div>
...
$(document).ready(function () {
    ...
    for (rowindex = 0; rowindex <= rownumber; rowindex++) {
        ...
        temp_unit_name = $('#product_table tbody tr:nth-child('+ (rowindex + 1) +')').find('.transfer-unit').val().split(',');
        unit_name.push($('#product_table tbody tr:nth-child('+ (rowindex + 1) +')').find('.transfer-unit').val());
        unit_operator.push($('#product_table tbody tr:nth-child('+ (rowindex + 1) +')').find('.transfer-unit-operator').val());
        unit_operation_value.push($('#product_table tbody tr:nth-child('+ (rowindex + 1) +')').find('.transfer-unit-operation-value').val());
        $('#product_table tbody tr:nth-child('+ (rowindex + 1) +')').find('.transfer-unit').val(temp_unit_name[0]);
    ...
    $('select[name="transfer_status"]').val($('input[name="transfer_status_hidden"]').val());
    $('select[name="received_by"]').val($('input[name="received_by_hidden"]').val());
