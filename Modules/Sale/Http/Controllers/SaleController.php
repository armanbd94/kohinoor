...
class SaleController extends BaseController
{
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'districts' => District::where('type',2)->get(),
            'upazilas'  => Upazila::where('type',2)->get(),
            'routes'    => Route::where('type',3)->get(),
        ];
        ...
        if($request->ajax()){
            ...
            try {
                $sale_data = [
                    ...
                    'asm_id'           => DB::table('salesmen')->where('id',$request->salesmen_id)->value('asm_id'),
                ...
                if($request->has('products'))
                {
                    foreach ($request->products as $key => $value) {
                        ...
                        $warehouse_finish_goods = WarehouseFinishGoods::where(['warehouse_id'=>$request->warehouse_id,'finish_goods_id'=>$value['id']])->first();
                ...
                $data = $this->sale_balance_add($result->id,$request->memo_no,$request->grand_total,$total_tax,$sum_direct_cost,$customer->coa->id,$customer->name,$request->sale_date,$payment_data,$request->warehouse_id);
    ...
    public function edit(int $id)
    {
            ...
            $data = [
                'sale'      => $sale_data,
                'warehouses' => Warehouse::where('status',1)->get(),
                'salesmen'  => Salesmen::where('asm_id',$sale_data->asm_id)->get(),
                'taxes'     => Tax::activeTaxes(),
            ...
            try {
                ...
                if(!$saleData->sale_products->isEmpty())
                {
                    foreach ($saleData->sale_products as  $sale_product) {
                        ...
                        $finishGoods = FinishGood::find($sale_product->id);
                        if($saleData->warehouse_id == 1)
                        {
                            $finishGoods->qty += $old_sold_qty;
                            $finishGoods->update();
                        }
                ...
                $customer = Customer::with('coa')->find($request->customer_id);
                $this->sale_balance_add($request->sale_id,$request->memo_no,$request->grand_total,$total_tax,$sum_direct_cost,$customer->coa->id,$customer->name,$request->sale_date,$payment_data,$request->warehouse_id);
        ...
        if($request->ajax()){
            if(permission('sale-delete'))
            {
                ...
                try {
                    ...
                    Transaction::where(['voucher_no'=>$request->id,'voucher_type'=>'INVOICE'])->delete();
            ...
            if(permission('sale-bulk-delete')){
                foreach ($request->ids as $id) {
                    ...
                    try {
                        ...
                        Transaction::where(['voucher_no'=>$id,'voucher_type'=>'INVOICE'])->delete();
    ...
    public function salesmen_list(Request $request)
    {
        if($request->ajax())
        {
            $output = '';
            $asm_id = DB::table('asms')->where('warehouse_id',$request->warehouse_id)->value('id');
            if($asm_id)
            {
                $salesmen = DB::table('salesmen')->where('asm_id',$asm_id)->get();
                if(!$salesmen->isEmpty())
                {
