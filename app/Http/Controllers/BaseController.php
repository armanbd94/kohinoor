...
class BaseController extends Controller
{
    ...
    protected function table_image($path,$image,$alt_text,$gender=null)
    {
        if(!empty($path) && !empty($image) && !empty($alt_text))
        {
            ...
            if($gender){
                if(is_int($gender)){
                    return "<img src='".asset("images/".($gender == 2 ? 'female' : 'male').".svg")."' alt='Default 2Image' style='width:50px;'/>";
                }else{
                    return "<img src='".asset("images/".($gender == 'Male' ? 'male' : 'female').".svg")."' alt='Default Image' style='width:50px;'/>";
                }
